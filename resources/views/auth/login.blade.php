            ...
            <div class="row g-0" style="min-height: 650px;">
                ...
                <div class="col-md-7 bg-white p-5 d-flex flex-column justify-content-center position-relative">
                    ...
                    <div class="mx-auto w-100" style="max-width: 420px;">
                        ...
                        <form action="{{ route('login.submit') }}" method="POST" id="loginForm" onsubmit="return validateLogin()">
                            @csrf

                            @if($errors->any())
                                <div class="alert alert-danger py-2 small fw-medium mb-4 rounded-3 border-0 bg-danger bg-opacity-10 text-danger" id="serverError">
                            ...
                            <div id="clientError" class="alert alert-danger py-2 small fw-medium mb-4 rounded-3 border-0 bg-danger bg-opacity-10 text-danger" style="display: none;">
...
<script>
    ...
    function validateLogin() {
        ...
        const serverErrors = document.querySelectorAll('#serverError');
        ...
        const commonWrongInputs = [
            'test', 'demo', '123', 'password123', 'admin', 'user', 'guest',
            'wrong', 'incorrect', 'fake', 'temp', 'temporary'
        ];

        if (commonWrongInputs.includes(password.toLowerCase()) ||
            commonWrongInputs.includes(email.toLowerCase().split('@')[0])) {
            showError('Invalid credentials. Please use correct email and password.');
            document.getElementById('password').focus();
            return false;
        }
    ...
    function showError(message) {
        ...
        clientError.style.animation = 'shake 0.5s';
        setTimeout(() => {
            clientError.style.animation = '';
        }, 500);
    }
